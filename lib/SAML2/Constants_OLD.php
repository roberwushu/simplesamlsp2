...
require_once('../../lib/SAML2/EidasRequestedAttribute.php');
require_once('../../lib/SimpleSAML/Utils/Random.php');
...
class Constants
{
    ...
    */
    ...
    /**
     * The namespace for the SAML eIDAS schema.
     */
     ...
     * The namespace for the SAML eIDAS schema.
     */
    const NS_EIDAS_NATURAL = 'http://eidas.europa.eu/attributes/naturalperson';
     ...
     * SP ID
     */
    const SPID = 'S2833002E_E04975701';
    ...
    /**
     * URL of the SP endpoint that will manage IdP assertions
     */
    const ASSERTION_URL = 'http://simplesaml2.local:8081/SP/return.php';

     /**
     * Whether the RelayState SAML attribute should be included in the request or not
     */
    const INCLUDE_RELAYSTATE_ATTR = true;	 

    const QAALEVEL = 'eidas:QualityAuthenticationAssuranceLevel';

    /*Tocar aquí para añadir atributos*/		
    public static $idsAttrs = array('AFirmaIdP', 'GISSIdP', 'AEATIdP', 'EIDASIdP', 'RelayState', 'CLVMOVILIdP' );
    public static $attrs = array(
		...
		'GISSIdP.value' => NULL,
        'AEATIdP.name' => 'AEATIdP',
		'AEATIdP.uri' => 'http://es.minhafp.clave/AEATIdP',
		'AEATIdP.nameFormat' => 'urn:oasis:names:tc:SAML:2.0:attrname-format:uri',
		'AEATIdP.value' => NULL,
    ...
    public static function genDefaultAttrs($request) {    
        ...
        $ce_type = $dom->createElementNS(Constants::NS_EIDAS, 'SPType', 'public');
        $extensions['eidas:SPType']= new \SAML2\XML\Chunk($ce_type);
        
        $attrCount=0;

        if (Constants::INCLUDE_RELAYSTATE_ATTR) {
            $relayStateValue=\SimpleSAML\Utils\Random::generateID();
            
            $ce0 = $dom->createElementNS(Constants::NS_EIDAS, 'AttributeValue', $relayStateValue);
            ...
            $ce1 = $dom->createElementNS(Constants::NS_EIDAS, 'RequestedAttribute');
            $ce1->setAttribute('FriendlyName', self::$attrs['RelayState.name']);
            $ce1->setAttribute('Name', self::$attrs['RelayState.uri']);
            $ce1->setAttribute('NameFormat', self::$attrs['RelayState.nameFormat']);
            $ce1->setAttribute('isRequired', 'false');
            
            $ce1->appendChild($ce0);
            $array_atributos[0]= new \SAML2\XML\Chunk($ce1);
            
            $attrCount++;
        }
        
        foreach(self::$idsAttrs as $key) {
            if ( $request[$key] === 'off' ) {        
                $ce1 = $dom->createElementNS(Constants::NS_EIDAS, 'RequestedAttribute', $attrValue);
                $ce1->setAttribute('FriendlyName', self::$attrs[$key.'.name']);
                $ce1->setAttribute('Name', self::$attrs[$key.'.uri']);
                $ce1->setAttribute('NameFormat', self::$attrs[$key.'.nameFormat']);
                $ce1->setAttribute('isRequired', 'false');
                $array_atributos[$attrCount]= new \SAML2\XML\Chunk($ce1);
                $attrCount++;
            }
        }
        
        if ($attrCount > 0) {    
            $extensions['RequestedAttributes'] = $array_atributos;
        }
        else {
            $extensions['RequestedAttributes'] = array();
        }
