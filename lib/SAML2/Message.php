...
require_once('../../lib/SAML2/SignedElement.php');
require_once('../../lib/SAML2/metadata_resp.php');
 ...
 */
abstract class SAML2_Message implements SignedElement
{
    /**
     * Request extensions.
     *
        ...
        foreach ($this->validators as $validator) {
           ...
           if(is_array($validator['Function']))
            ...
            try {
                ...
                Utils::validateSignature($data, $key);
               // "validateSignature";
    ...
    public function toUnsignedXML()
    {
        ...
        $root = $this->document->createElementNS(Constants::NS_SAMLP, 'saml2p:'.$this->tagName); 
        ...
        $root->setAttributeNS(Constants::NS_EIDAS_NATURAL, 'eidas-natural:tmp', 'tmp');
        $root->removeAttributeNS(Constants::NS_EIDAS_NATURAL, 'tmp');
    ...
    public function toUnsignedXMLLogOut()
    {
        $this->document = DOMDocumentFactory::create();
        $root = $this->document->createElementNS(Constants::NS_SAMLP, 'saml2p:'.$this->tagName); 
        $this->document->appendChild($root);
    ...
    public static function fromXML_MET(\DOMElement $xml)
    {
      
        if ($xml->namespaceURI !== Constants::NS_SAMLP_MET) {
            throw new \Exception('Unknown namespace of SAML message: '.var_export($xml->namespaceURI, true));
        }
