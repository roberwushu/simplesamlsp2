...
class SimpleSAML_Metadata_MetaDataStorageHandler
{
     ...
     */
    private static $localConfigHandler = null;
     ...
     */
    public static function getlocalConfigHandler()
    {
        if (self::$localConfigHandler === null) {
            self::$localConfigHandler = new SimpleSAML_Metadata_MetaDataStorageHandler();
        }

        return self::$localConfigHandler;
    }
    ...
    public function getGenerated($property, $set)
    {
        ...
        try {
            $localConfigSet = $this->getlocalConfigCurrent($set);
            if (array_key_exists($property, $localConfigSet)) {
                return $localConfigSet[$property];
            }
        } catch (Exception $e) {
    ...
    public function getList($set = 'saml20-idp-remote')
    {
        ...
        foreach ($this->sources as $source) {
            $srcList = $source->getlocalConfigSet($set);
     ...
     */
    public function getlocalConfigCurrent($set)
    {
        return $this->getLocalConfig(null, $set);
    }
     ...
     */
    public function getlocalConfigCurrentEntityID($set, $type = 'entityid')
    {
        assert(is_string($set));
     ...
     */
    public function getLocalConfig($index, $set)
    {
        assert(is_string($set));

        if ($index === null) {
            $index = $this->getlocalConfigCurrentEntityID($set, 'metaindex');
        }

        assert(is_string($index));

        foreach ($this->sources as $source) {
            $localConfig = $source->getLocalConfig($index, $set);

            if ($localConfig !== null) {

                if (array_key_exists('expire', $localConfig)) {
                    if ($localConfig['expire'] < time()) {
                        throw new Exception(
                            'Metadata for the entity ['.$index.'] expired '.
                            (time() - $localConfig['expire']).' seconds ago.'
                        );
                    }
                }
                $localConfig['metadata-index'] = $index;
                $localConfig['metadata-set'] = $set;
                assert(array_key_exists("entityid", $localConfig));
                return $localConfig;
            }
        }
    ...
    public function getlocalConfig_idp($index, $set=null, $destination=null)
    {
        assert(is_string($set));
...
//        assert(is_string($index));
           ...
           // $localConfig = $source->getLocalConfig($index, $set);
            ...
            if ($index !== null) {
                ...
                if (array_key_exists('expire', $index)) {
                    if ($localConfig['expire'] < time()) {
                        throw new Exception(
                            'Metadata for the entity ['.$index.'] expired '.
                            (time() - $localConfig['expire']).' seconds ago.'
                ...
                $localConfig['Location'] = $index[0];
                $localConfig['Binding'] = $index[1];
                $localConfig['SingleSignOnService'] = $index[0];
                $localConfig['metadata-index'] = $index;
                $localConfig['metadata-set'] = $set;
//                assert(array_key_exists("entityid", $localConfig));
                return $localConfig;
     ...
     * @throws SimpleSAML_Error_MetadataNotFound If no metadata for the entity specified can be found.
     */
    public function getlocalConfigConfig($entityId, $set)
    {
        assert(is_string($entityId));
        assert(is_string($set));

        $localConfig = $this->getLocalConfig($entityId, $set);
        return SimpleSAML_Configuration::loadFromArray($localConfig, $set.'/'.var_export($entityId, true));
    }
    
    public function getlocalConfigConfig_meta($entityId, $set)
    {
//        assert(is_string($entityId));
        assert(is_string($set));

        $localConfig = $this->getlocalConfig_idp($entityId, $set);
        return SimpleSAML_Configuration::loadFromArray($localConfig, $set.'/'.var_export($entityId, true));
    }
     ...
     * found.
     */
    public function getlocalConfigConfigForSha1($sha1, $set)
    {
