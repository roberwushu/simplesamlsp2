...
class HTTPArtifact extends Binding
{
    ...
    public function receive()
    {
        ...
        $idplocalConfig = $localConfigHandler->getlocalConfigConfigForSha1($sourceId, 'saml20-idp-remote');

        if ($idplocalConfig === null) {
            throw new \Exception('No metadata found for remote provider with SHA1 ID: ' . var_export($sourceId, true));
        }

        $endpoint = null;
        foreach ($idplocalConfig->getEndpoints('ArtifactResolutionService') as $ep) {
        ...
        /* Sign the request */
        \sspmod_saml_Message::addSign($this->spMetadata, $idplocalConfig, $ar); // Shoaib - moved from the SOAPClient.
